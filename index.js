    ...
    const updateFn = (currentTags) => {
        ...
        replaceFrames.forEach((frameIdentifier) => {
            if(Object.prototype.hasOwnProperty.call(rawTags, frameIdentifier) ||
                (frameIdentifier === 'APIC' && Object.prototype.hasOwnProperty.call(rawTags, 'images'))) {
                delete currentTags[frameIdentifier]
            }
        })
        Object.keys(rawTags).map((frameIdentifier) => {
            const controlledFrameIdentifier = frameIdentifier === 'images' ? 'APIC' : frameIdentifier
            if(removeFrames.has(controlledFrameIdentifier)) {
                return
            }
            const options = ID3Util.getSpecOptions(controlledFrameIdentifier, 3)
            const cCompare = {}
